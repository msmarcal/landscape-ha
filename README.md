...
A self-signed certificate is automatically generated by Terraform using the TLS provider.
Configure the certificate CN and SANs:

```hcl
ssl_cert_cn = "landscape.maas"
ssl_cert_sans = ["landscape.maas", "landscapeha-1.maas"]
```

The certificate is passed to HAProxy and landscape-server via charm config, and exported
to `ssl_cert_export_path` for use by Landscape clients.

For production, override with your own certificate in the haproxy config:
